    ...
    <div class="mr-12">
        <div class="mr-3">
            ...
            >
                ...
                <option v-for="primary_category in primary_categories" :key="primary_category.id"
                        :value="primary_category.id">
        ...
        <div class="mr-3">
            ...
            >
                ...
                <option v-for="secondary_category in secondary_categories" :key="secondary_category.id"
                        :value="secondary_category.id">
        ...
        <div class="mr-3" v-for="(item, index) in textBoxes" :key="index">
            GPTクエリ :
            <div class="relative mr-6 flex flex-wrap items-stretch">
                ...
                <button
                    ...
                    type="button"
                    @click="writeToClipboard(item.id)"
                    id="button-addon2">
...
export default {
    setup(props) {
        const textBoxes = [{id: '1',value: 'サンプル1'},{id: '2',value: 'サンプル1'},{id: '3',value: 'サンプル2'}];
    ...
    methods: {
        async onCategoryChange() {
            ...
            try {
                const gpt_tpl = await axios.get(`/api/examination/get_data/`)
                    .then(response => {
                        this.examinations = response.data;

                    });
                for (const item of this.examinations) {
                    const response = await axios.get(`/api/examination/get_gpt/${item.id}/${this.selected_category_id}`);
                    this.gpt_str[item.id] = response.data;
                }


                //const response = await axios.get(`/api/category/get_gpt/${this.selected_category_id}`);
                //this.gpt_str[0] = response.data;
                //console.log(this.gpt_str[0]);
                //this.$emit('category-selected', this.selected_category_id);
            } catch (error) {
                console.error(error);
                this.gpt_str[0] = ''; // Reset or handle error
            }
        },
        ...
        writeToClipboard(id) {
            const copyText = this.gpt_str[id]
            navigator.clipboard
                .writeText(copyText)
                .then(() => {
